...
<div style="background-color: white;  margin: 30px; margin-right: 55px; padding:30px;">
<form:form modelAttribute="authUser" action="${cPath}/mypage/myInfo/myEmpInformationUpdate" method="post" enctype="multipart/form-data"  id="inputForm" style="padding : 5px">
	<table class="table table-bordered text-center">
		<thead>
			<tr>
				<td rowspan="5" colspan="2" style="padding:auto;">
					...
					<tr>
						<td><!-- 이미지파일 불러오는거  -->
							<img id="emp_img" class='emp_img' style="width: 150px; max-heigth: 100px;" alt="" src="${cPath }/resources/empImg/${authUser.emp_no}">
						</td>
					</tr>
					</table>
					<input class="form-control" type="file" name="emp_img" accept="image/*" id="input_img"  disabled="disabled">
					
				</td>
				<th colspan="2">사원아이디</th>
				<td colspan="5" id="emp_no">${authUser.emp_no}
			...
			<tr>
				...
				<td colspan="5">${authUser.emp_pass}<input class="col-8 form-control mr-2 code " type="hidden" name='emp_pass' value="${authUser.emp_pass}"/></td>
			</tr>
			<tr>
				<th colspan="2">이름</th>
				<td colspan="2"><input class="col-12 form-control mr-2" type="text"
					name="emp_name" value="${authUser.emp_name}" readonly ></td>
				
				<th colspan="2">영문이름</th>
				<td colspan="2"><input class="col-12 form-control mr-2" type="text"
					name="emp_en_name" value="${authUser.emp_en_name }"  readonly></td>
			</tr>
			<tr>	
				<th colspan="2">생일</th>
				<td colspan="5"><input class="col-5 form-control mr-2"
					type="date" name="emp_birthday" value="${authUser.emp_birthday}"  readonly>
				</td>
			</tr>
			<tr>
				<th colspan="2">주민번호</th>
				<td colspan="5">
				<input style="text-align" class="col-5 form-control mr-2" type="text" name="emp_regno" value="${authUser.emp_regno }" readonly>  
		...
		<tbody>
			<tr>
				<th>부서</th>
				<td><select class="dynamicElement form-control mr-2 code "
					name="dep_no2"   disabled data-url="${cPath}/emp/manage/departmentList">
						<option >${authUser.depVO.dep_no2} </option>
					</select>
				</td>
				<th>소속</th>
				<td><select class="dynamicElement form-control mr-2 code"
					name="dep_no"  disabled data-url="${cPath}/emp/manage/TeamList">
						<option value="${authUser.depVO.dep_no}">${authUser.depVO.dep_name}</option>
				</select></td>
				<th>직급</th>
				<td><select class=" form-control mr-2 code" 
					name="pos_no"  disabled>
						<c:forEach items="${posList}" var="posList" > 
							<option value="${posList.POS_NO}" ${authUser.pos_no eq posList.POS_NAME ? "selected" : ""}> ${posList.POS_NAME}</option>
						</c:forEach>
				</select></td>
				<th>입사일</th>
				<td colspan="2"><input class="col-12 form-control mr-2 code" type="date" name="hire_date" value="${authUser.hire_date }" disabled> 
				<input type="hidden"  value="${authUser.hire_date }"  name="hire_date">
			...
			<tr>
				<th>성별</th>
				<td><input class="code" type="radio" name="emp_gen"value="남성" disabled="disabled"
					${'남성' eq authUser.emp_gen?"checked" :""} /> 남성<input class="code"
					type="radio" name="emp_gen" value="여성"  disabled="disabled"
					${'여성' eq authUser.emp_gen?"checked" :""} /> 여성<span class="error">${errors["emp_gen"] }</span>
				</td>
				<th>국적</th>
				<td><input class="code"   type="radio" name="country" value="내국인" disabled="disabled"
					${'내국인' eq authUser.country?"checked" :""} /> 내국인
					
					 <input class="code" type="radio" name="country" value="외국인" disabled="disabled"
					${'외국인' eq authUser.country?"checked" :""} /> 외국인 <span class="error">${errors["country"] }</span>
				</td>
				...
				<td><select class=" form-control mr-2"
					name="soldier" disabled>
						<c:forEach items="${solList}" var="solList">
							<option value="${solList.CODE}" ${authUser.soldier eq solList.CODE ? "selected" : "" }>${solList.NAME}</option>
						</c:forEach>
				</select></td>

				<th>구분</th>
				<td colspan="2"><select
					class=" form-control mr-2 code" name="fulltime" disabled >
						<c:forEach items="${fullList}" var="fullList">
							<option value="${fullList.CODE }" ${authUser.fulltime eq fullList.CODE ? "selected" : ""}>${fullList.NAME}</option>
						</c:forEach>
				</select>
				<input type="hidden"  value="${authUser.fulltime }"  name="fulltime">
				</td>
			...
			<tr>
				<th>은행명</th>
				<td colspan="2"><select class=" form-control mr-2 code" name="bank" disabled>
						<option value="신한" ${authUser.bank eq "신한" ? "selected" : ""}>신한</option>
						<option value="국민" ${authUser.bank eq "국민" ? "selected" : ""}>국민</option>
						<option value="농협" ${authUser.bank eq "농협" ? "selected" : ""}>농협</option>
						<option value="우리" ${authUser.bank eq "우리" ? "selected" : ""}>우리</option>
						<option value="기업" ${authUser.bank eq "기업" ? "selected" : ""}>기업</option>
						<option value="새마을금고" ${authUser.bank eq "새마을금고" ? "selected" : ""}>새마을금고</option>
				</select>
				<input type="hidden"  value="${authUser.bank }"  name="bank">
				</td>
				<th>예금주</th>
				<td colspan="2"><input class="col-12 form-control mr-2 code"  value="${authUser.emp_name}" type="text" readonly /></td>
				<th>계좌번호</th>
				<td colspan="2"><input class="col-12 form-control mr-2 code" type="text" name="emp_bankno"
					value="${authUser.emp_bankno }"  readonly /></td>
			</tr>
			<tr>
				...
				<td colspan="2"><input class="col-12 form-control mr-2"
					type="text" name="emp_email"  readonly value="${authUser.emp_email }"> <span
					class="error">${errors["emp_email"] }</span></td>

				<th>연락처</th>
				<td colspan="2"><input class="col-12 form-control mr-2"
					type="text" name="emp_phone" readonly  value="${authUser.emp_phone }"> <span
					class="error">${errors["emp_phone"] }</span></td>
			</tr>
			<tr>
				<td colspan="4"><input class="form-control"
					style="width: 50%; display: inline;" placeholder="우편번호" value="${authUser.emp_zip}"
				...
				<td colspan="3"><input class="form-control" style="top: 5px;" value="${authUser.emp_addr}"
					placeholder="도로명 주소" name="emp_addr" id="addr2" type="text"
					readonly="readonly" /></td>
				<td colspan="3"><input class="form-control" placeholder="상세주소"  value="${authUser.emp_addr2}"
					readonly="readonly"  name="emp_addr2" id="addr3" type="text" /></td>
			</tr>
		<tbody id="trsch">
		 <c:forEach var="eduVOList" items="${authUser.eduVOList}" begin="0" step="1" varStatus="i">
			<tr id="schoolName">
				<th>학교명</th>
		...
		<tbody id="trlic">
		 <c:forEach items="${authUser.empCertVOList}" var="empCertVOList"  begin="0" step="1" varStatus="i">
			<tr id="licence">
				<th>자격증구분</th>
		...
		<tbody id="trcom">
		 ...
		 <c:forEach items="${authUser.careerVOList}" var="careerVOList" begin="0" step="1" varStatus="i">
			<tr id="company">
				<th>회사명</th>
...
$("form").find('.form-control').not('.code').attr('disabled','disabled');
$("form").find("input[name='btn']").not('.code').attr('disabled','disabled');
var upBtn = $("#upBtn").on(
		"click",
		function(event){
			if('수정' == upBtn.text()){
				upBtn.text('등록');
				$("form").find('.form-control').not('.code').removeAttr('readonly');
				$("form").find('.form-control').not('.code').removeAttr('disabled'); // disable 를 지운다. 
				$("form").find("input[name='emp_gen']").not('.code').removeAttr('disabled');
				$("form").find("input[name='country']").not('.code').removeAttr('disabled');
				$("form").find("input[name='btn']").not('.code').removeAttr('disabled');
				$("form").find("button[name='seach']").not('.code').removeAttr('disabled');
				return;
			}
			...
			console.log(form);
			$.ajax({
				url : "${cPath}/mypage/myInfo/myEmpInformationUpdate",
				processData : false,
				contentType : false,
				type : 'post',
				data : data,
