...
<h2 style="padding-left: 30px; padding-top: 25px;">구매요청서</h2>

<form id="docForm" action="${cPath}/buy/document" method="post">
	<div style="padding-left: 30px; padding-top: 10px;">
		납기 요청일자 : <input type="date" id="docDate" name="doc_inDate"/>
		<input type="button" id="matBtn" class="btn btn-info" value="원자재 추가" style="margin-left:10px;height:30px;padding-top:2px"/>
		...
		<span style="margin-left:10px">'원자재추가' 버튼을 클릭하여, 요청목록을 추가하세요!</span>
	</div>
	<div>
		<form:errors path="docVO.buyer_sector" element="span" cssClass="error" />
	</div>
	<div style="background-color: white; margin: 20px 30px 10px 30px; margin-right: 55px;">
	...
	<div style="padding-left: 30px; padding-top: 10px;">
		<input type="button" id="delBtn" class="btn btn-outline-danger" value="선택삭제"/>
		<div class="btnStyle" style="margin-top:-40px;">
			<input type="button" id="regBtn" class="btn btn-primary" value="등록" style="width:100px;"/>
		</div>
	</div>
</form>
...
<script type="text/javascript">
	...
	$(document).on("click", "#matAddBtn", function(){
		$("input[name=checkMat]:checked").each(function() {
			...
			let str = $("<tr/>").addClass('trTag').append(
					...
					$("<td>").html($("<input/>").addClass("docQty").attr({type:"number", name:"matList["+index+"].qty"}))
			);
	...
	$("#regBtn").click(function(event){
		...
		$.each($(".docQty"),function(index, doc_qty) {
			if($(doc_qty).val() == ''){
