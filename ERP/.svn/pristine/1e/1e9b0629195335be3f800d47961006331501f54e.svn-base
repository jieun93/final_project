...
<form id="orderForm" action="${cPath}/buy/order" method="post">

<div class="middleDiv">
	<div class="input-group mb-3">
		<input type="button" id="matBtn" value="원자재 추가" class="btn btn-info" style="height:32px;padding-top:2px"/>&nbsp;
		<input type="button" onclick="addRow()" value="일반발주 추가" class="btn btn-info" style="height:32px;padding-top:2px"/>
		<div style="margin:auto 5px; margin-left:20px;">납기요청일자</div> 
		<input id="or_date" type="date" name="req_date" style="margin-right:10px;"/>
...
<!-- 	<input type="hidden" id="insertError" value="insert 오류"/> -->
	...
	<!-- ------------------------------------------------------------------------------------ -->
	<h5 style="padding-left:550px; padding-top:5px; margin-bottom:-10px;">&lt;구매요청 원자재목록&gt;</h5>
		  	...
		  	<c:if test="${not empty docMatList }">
		  		<c:forEach items="${docMatList}" var="docMat">
		  			<tr>
			  			...
			  			<th><input type="button" class="docDelBtn btn btn-outline-danger" value="삭제" style="width:80px;height:30px;padding-top:2px"/></th>
		  			</tr>
		  		</c:forEach>
		  	</c:if>
	...
	<div class="middleDiv">
		...
		<input type="button" id="docMoveBtn" class="btn btn-outline-success" value="발주이동"/>
	</div>
	<!-- ------------------------------------------------------------------------------------ -->
...
<!-- 	<div class="middleDiv"> -->
<!-- 		발주 총액 : <span id="or_totalPrice"></span> -->
<!-- 	</div> -->
	<div class="middleDiv">
		<input type="checkbox" id="allChk"/><label for="allChk">모두 선택</label>
		<input type="button" id="delBtn" value="선택삭제" class="btn btn-outline-danger"/><br/>
		<div style="position:absolute; right:70px;margin-top:-40px;">
			발주 총액 : <span id="or_totalPrice">0</span>원
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="button" id="regBtn" value="발주등록" class="btn btn-primary" style="width:100px;"/>
		</div>
	</div>
	<br><br>
</form>
...
<script type="text/javascript">
	...
	$("#docMoveBtn").on("click", function(){
		...
		$("input[class=docCheck]:checked").each(function() {
			...
			trTag += "<td><input type='number' name='matList["+index+"].cost' class='or_cost'/></td>";	//단가
			trTag += "<td><input type='number' class='or_price'/></td></tr>";	//공급가액
	...
	function addRow(){
		...
		str += "<td><input type='number'/></td></tr>";	//공급가액
...
// 	}
	...
	$(document).on("click", "#matAddBtn", function(){
		...
		$("input[name=checkMat]:checked").each(function() {
			...
			let mat_in_cost = $(this).parent().parent().find(".mat_in_cost").text();
			...
			let str = $("<tr/>").addClass('trTag').append(
					...
					$("<td>").addClass('inputStyle').html($("<input/>").addClass("qty").attr({type:"number", name:"matList["+index+"].ormat_qty"})),
					$("<td>").addClass('inputStyle').html($("<input/>").addClass("or_cost").attr({type:"number", name:"matList["+index+"].cost", value:mat_in_cost})),
					$("<td>").addClass('inputStyle').html($("<input/>").addClass("or_price").attr({type:"number"}))
			);
	...
	//공급가액 자동 계산 -----------------------------------------
	$("#formTable").on("change keyup paste", "input", function(){
		let qty = $(this).parent().parent().find(".qty").val();
		let cost = $(this).parent().parent().find(".or_cost").val();
		
		//공급가액 입력
		$(this).parent().parent().find(".or_price").val(qty*cost);
		
		//총금액
		var or_totalPrice = 0;
		$.each($(".or_price"), function(index, or_price){
			var or_price = $(or_price).val() * 1;
				or_totalPrice += or_price;
		});
		$("#or_totalPrice").text(or_totalPrice);
	});
...
// 		event.preventDefault();	//submit 이벤트 막기
		...
		$.each($("#orderBody .qty"),function(index, qty) {
			if($(qty).val() == ''){
				alert((index+1)+"번째 원자재 수량을 입력해주세요");
				flag = false; return false;
			}
		});
		$.each($("#orderBody .or_cost"),function(index, cost) {
			if($(cost).val() == ''){
				alert((index+1)+"번째 원자재 단가를 입력해주세요");
				flag = false; return false;
			}
		});
		if(flag){
