...
//  let names = new Set(data.map(d=>d.name));
  ...
  let rank = function rank(value){
    const data = Array.from(names, name => ({name, value: value(name)}));
    data.sort((a, b) => d3.descending(a.value, b.value));
    for (let i = 0; i < data.length; ++i) data[i].rank = Math.min(n, i);
    return data;
  }
  let keyframes = (function(){
    const keyframes = [];
    let ka, a, kb, b;
  ...
  let textTween = function textTween(a, b) {
  ...
  let labels = function labels(svg) {
  ...
  let ticker = function ticker(svg) {
  ...
  let chart = async function(){
    const svg = d3.select(renderArea)
      .append("svg")
