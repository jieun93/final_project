// https://observablehq.com/@d3/bar-chart-race@3042
export default function define(runtime, observer, year) {
	console.log(runtime);
	console.log(observer);
  const main = runtime.module();
  const fileAttachments = new Map([["category-brands.csv",new URL("/ERP/account/pm/product/"+2020,import.meta.url)]]);
...
d3.csvParse(await FileAttachment("category-brands.csv").text(), d3.autoType)
  ...
  main.variable().define("viewof replay", ["html"], function(html){
	  console.log(html);
	  return(
html`<button onclick="a();">검색`
)});
